...
const UpdateServiceWork = ({ data, setData }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true)
            await api.vfCv2Axios.put(`/config/service-work/${data?.work_uuid}`, form)
            setData((state) => state?.map((work) => {
                if (work?.work_uuid === data?.work_uuid) {
                    return {
                        ...work,
    ...
    useEffect(() => {
        setForm({
            ...
            service_scope: data?.['Scop'],
            selling_rate: String(data?.['Selling Rate']),
            discount_rate: String(data?.['Discount Rate']),
            purchase_rate: String(data?.['Purchase Rate']),
            call_rate: String(data?.['Call Rate']),
            refill_included: data?.refill_included,
            reinstallation_included: data?.reinstallation_included
        })
    }, [data])
    ...
    return (
        <div className="service-work-form-container">
            <form onSubmit={handleSubmit}>
                ...
                <Button label={'Update'} rounded spinIcon={loading} severity='primary' style={{ width: '100%' }} />
...
export default UpdateServiceWork